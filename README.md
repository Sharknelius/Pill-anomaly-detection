...
### General Setup
This codebase was make with Python 3.12.8. Any Python 3.12 version should be compatible. Other Python 3 versions may work too.
...
- `new_name`: This is automatically set in the code as the date and time the `train_with_pseudo_labels.py` is ran plus `_best_frcnn.pt`. This is the string name of the output model.
- `not_semi`: A boolean value that if set TRUE, will train the model with the supervised approach using only labeled data. If FALSE, then the model will be trained on the semi-supervised/self-learning approach.
